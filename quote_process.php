...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($_POST['event']) || !is_array($_POST['event'])) {
        $errors['event'] = 'Select at least one Event.';
    }
    if (empty($_POST['technical']) || !is_array($_POST['technical'])) {
        $errors['technical'] = 'Select at least one Technical Production option.';
    }
    if (empty($_POST['others']) || !is_array($_POST['others'])) {
        $errors['others'] = 'Select at least one Others Solution.';
    }
    ...
    if (empty($errors)) {
        ...
        $sanitized_event = implode(', ', array_map('mysqli_real_escape_string', array_fill(0, count($_POST['event']), $conn), $_POST['event']));
        $sanitized_technical = implode(', ', array_map('mysqli_real_escape_string', array_fill(0, count($_POST['technical']), $conn), $_POST['technical']));
        $sanitized_others = implode(', ', array_map('mysqli_real_escape_string', array_fill(0, count($_POST['others']), $conn), $_POST['others']));
        ...
        $sql = "INSERT INTO client_quote_request (client_name, designation, orgname, country, email, phone_number, status, 
                event_interest, technical_interest, others_interest, comments, agree, privacy_agreement) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
